...
public class BemServiceImpl implements BemService{
    ...
    public Bem findById(int id) {
    	try {
    		Bem bem = repository.findOne(id);
        	String dataAtual = dataAtual();
			String dataAquisicao = dataAquisFormat(bem.getDataAquis());
			int anos = quantidadeAnosDiferenca(dataAquisicao, dataAtual);
			bem.setValorAtual(bem.getValorCompra());
			float valorAtual = calculoDepcreciacao(bem.getValorCompra(), bem.getValorAtual(), bem.getGrupoMaterial().toString(), anos);
